 ...
 * \param[in] pull_mode Pin pull mode
 */
static inline void RN_RX_set_pull_mode(const enum port_pull_mode pull_mode)
{
	PORTD_set_pin_pull_mode(0, pull_mode);
}

/**
 * \brief Set RN_RX data direction
 *
 * Select if the pin data direction is input, output or disabled.
 * If disabled state is not possible, this function throws an assert.
 *
 ...
 *                      (low power state)
 */
static inline void RN_RX_set_dir(const enum port_dir dir)
{
	PORTD_set_pin_dir(0, dir);
}

/**
 * \brief Set RN_RX level
 *
 * Sets output level on a pin
 *
 * \param[in] level true  = Pin level set to "high" state
 *                  false = Pin level set to "low" state
 */
static inline void RN_RX_set_level(const bool level)
{
	PORTD_set_pin_level(0, level);
}

/**
 * \brief Toggle output level on RN_RX
 *
 * Toggle the pin level
 */
static inline void RN_RX_toggle_level()
{
	PORTD_toggle_pin_level(0);
}

/**
 * \brief Get level on RN_RX
 *
 * Reads the level on a pin
 */
static inline bool RN_RX_get_level()
{
	return PORTD_get_pin_level(0);
}

/**
 * \brief Set RN_TX pull mode
 *
 * Configure pin to pull up, down or disable pull mode, supported pull
 * modes are defined by device used
 *
 * \param[in] pull_mode Pin pull mode
 */
static inline void RN_TX_set_pull_mode(const enum port_pull_mode pull_mode)
{
	PORTD_set_pin_pull_mode(1, pull_mode);
}

/**
 * \brief Set RN_TX data direction
 *
 * Select if the pin data direction is input, output or disabled.
 * If disabled state is not possible, this function throws an assert.
 *
 ...
 *                      (low power state)
 */
static inline void RN_TX_set_dir(const enum port_dir dir)
{
	PORTD_set_pin_dir(1, dir);
}

/**
 * \brief Set RN_TX level
 *
 * Sets output level on a pin
 *
 * \param[in] level true  = Pin level set to "high" state
 *                  false = Pin level set to "low" state
 */
static inline void RN_TX_set_level(const bool level)
{
	PORTD_set_pin_level(1, level);
}

/**
 * \brief Toggle output level on RN_TX
 *
 * Toggle the pin level
 */
static inline void RN_TX_toggle_level()
{
	PORTD_toggle_pin_level(1);
}

/**
 * \brief Get level on RN_TX
 *
 * Reads the level on a pin
 */
static inline bool RN_TX_get_level()
{
